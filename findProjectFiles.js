...
function findProjectFiles(filename, {projectDir, onlyDirectories, noDirectories, within, ignoreSubProjects}={}) {
    ...
    assert.internal(paths_found.every(path_found => path_module.isAbsolute(path_found)));
    return paths_found;
}
...
function find_up(filename, {onlyDirectories, noDirectories, cwd}) {
    ...
    */
    ...
    assert.internal(!found_path || found_path.constructor===String && path_module.isAbsolute(found_path));
