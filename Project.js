...
function Project() {
  ...
  assert.internal(
    userScript === null || (userScript && path.isAbsolute(userScript)),
    { userScript }
  );
  const userDir = userScript ? path.dirname(userScript) : process.cwd();
  ...
  function findConfigFile(...args) {
    ...
    file = files[0] || null;
    assert.internal(
      file === null || (path.isAbsolute(file) && !isOutsideProject(file)),
      { projectDir, file }
    );
    ...
    function sortByDistance(files) {
      files = files.sort((file1, file2) => {
        ...
        assert.internal(
          distance2.constructor === Number && distance2 !== distance1,
          { file1, file2 }
        );
        return distance1 - distance2;
      });
      assert.internal(
        !files[0] || !files[1] || getDistance(files[0]) < getDistance(files[1])
      );
      return files;
    }
  ...
  function isOutsideProject(file) {
    const isOutsideProject = !isAncestorDirectory(
      projectDir,
      path.dirname(file)
    );
  }
...
function isAncestorDirectory(ancestor, descendant) {
  ...
  const descendant_dirs = descendant
    .split(path.sep)
    .filter((dir) => dir !== "");
